...
def main():
    ...
    pre_micro = float(precision_score(y_test, y_pred, average='micro', zero_division=0))
    rec_micro = float(recall_score(y_test, y_pred, average='micro', zero_division=0))
    f1_macro = float(f1_score(y_test, y_pred, average='macro'))
    pre_macro = float(precision_score(y_test, y_pred, average='macro', zero_division=0))
    ...
    logger.info(f"[+] Test accuracy={acc:.4f}, micro-F1={f1_micro:.4f}, micro-Precision={pre_micro:.4f}, micro-Recall={rec_micro:.4f}")
    logger.info(f"[+]                        macro-F1={f1_macro:.4f}, macro-Precision={pre_macro:.4f}, macro-Recall={rec_macro:.4f}")
