...
def _path_major_train(label: str) -> str:
    safe = cic2018.get_label_name(label)
    return os.path.join(RAW_DIR, 'train', f"cic2018_{safe}_raw_processed_compressed.csv")


def _path_major_test(label: str) -> str:
    safe = cic2018.get_label_name(label)
    return os.path.join(RAW_DIR, 'test', f"cic2018_{safe}_raw_processed_compressed.csv")


def _path_major_train_kmeanspca_all(label: str) -> str:
    safe = cic2018.get_label_name(label)
    # Produced by kmeans_pca_merge_tt.py
    return os.path.join(RAW_DIR, 'train', f"cic2018_{safe}_raw_processed_compressed_kmeanspca_all.csv")


def _path_major_test_kmeanspca_all(label: str) -> str:
    safe = cic2018.get_label_name(label)
    # Produced by kmeans_pca_merge_tt.py
    return os.path.join(RAW_DIR, 'test', f"cic2018_{safe}_raw_processed_selected_kmeanspca_all.csv")
...
def main():
    parser = argparse.ArgumentParser(description="Merge CIC2018 major+minor raw datasets into single train/test CSVs")
    ...
    parser.add_argument('--major-kmeanspca-labels', '-K', type=str, nargs='+', default=None, choices=cic2018.MAJORITY_LABELS,
                        help='Labels to read from kmeanspca_all outputs for major (fallback to default if missing)')
    parser.add_argument('--major-clean-merged-train', type=str, nargs='+', default=None, choices=cic2018.MAJORITY_LABELS,
                        help='Major labels to read from clean_merged TRAIN instead of raw_processed/kmeanspca_all')
    parser.add_argument('--major-clean-merged-test', type=str, nargs='+', default=None, choices=cic2018.MAJORITY_LABELS,
                        help='Major labels to read from clean_merged TEST instead of raw_processed/kmeanspca_all')
    parser.add_argument('--log-level', type=str, default='INFO', choices=['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'])
    args = parser.parse_args()
    setup_logging(args.log_level)

    logger.info("[+] Merging TRAIN: major raw_processed + minor augmented raw_processed …")
    train_parts: list[pd.DataFrame] = []
    # Major train
    for lbl in cic2018.MAJORITY_LABELS:
        # Precedence: kmeanspca_all > clean_merged > default raw_processed
        p = _path_major_train(lbl)
        picked = 'raw_processed'
        if args.major_kmeanspca_labels and lbl in args.major_kmeanspca_labels:
            kp = _path_major_train_kmeanspca_all(lbl)
            if os.path.exists(kp):
                p = kp
                picked = 'kmeanspca_all'
                logger.info(f"[+] Using kmeanspca_all TRAIN for {lbl}")
        if picked != 'kmeanspca_all' and args.major_clean_merged_train and lbl in args.major_clean_merged_train:
            cp = _path_major_train_clean_merged(lbl)
            if os.path.exists(cp):
                p = cp
                picked = 'clean_merged'
                logger.info(f"[+] Using clean_merged TRAIN for {lbl}")
        if os.path.exists(p):
            try:
                df = pd.read_csv(p, low_memory=False)
                df['__source__'] = f"major:{lbl}"
                train_parts.append(df)
                logger.info(f"[+] Major train OK: {lbl} ({len(df)})")
            except Exception as e:
                logger.warning(f"[!] Skip major train {lbl}: {e}")
        else:
            logger.debug(f"[-] Missing major train: {p}")
    ...
    logger.info("[+] Merging TEST: major raw_processed + minor test clean_merged …")
    test_parts: list[pd.DataFrame] = []
    # Major test
    for lbl in cic2018.MAJORITY_LABELS:
        # Precedence: kmeanspca_all > clean_merged > default raw_processed
        p = _path_major_test(lbl)
        picked = 'raw_processed'
        if args.major_kmeanspca_labels and lbl in args.major_kmeanspca_labels:
            kp = _path_major_test_kmeanspca_all(lbl)
            if os.path.exists(kp):
                p = kp
                picked = 'kmeanspca_all'
                logger.info(f"[+] Using kmeanspca_all TEST for {lbl}")
        if picked != 'kmeanspca_all' and args.major_clean_merged_test and lbl in args.major_clean_merged_test:
            cp = _path_major_test_clean_merged(lbl)
            if os.path.exists(cp):
                p = cp
                picked = 'clean_merged'
                logger.info(f"[+] Using clean_merged TEST for {lbl}")
        if os.path.exists(p):
            try:
                df = pd.read_csv(p, low_memory=False)
                df['__source__'] = f"major:{lbl}"
                test_parts.append(df)
                logger.info(f"[+] Major test OK: {lbl} ({len(df)})")
            except Exception as e:
                logger.warning(f"[!] Skip major test {lbl}: {e}")
        else:
            logger.debug(f"[-] Missing major test: {p}")
